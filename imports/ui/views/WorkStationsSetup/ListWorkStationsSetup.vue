<template>
  <v-container>
    <v-row>
      <v-col>
        <div class="headline">Catálogo de configuración de estaciones de trabajo</div>
      </v-col>
      <v-col cols="2">
        <div class="d-flex flex-row-reverse mb-5">
          <v-tooltip bottom>
            <template v-slot:activator="{on}">
              <v-btn v-can:create.hide="'workstationsetup'" color="success" v-on="on" fab dark :to="{name:'home.workstationconfiguration.create'}">
                <v-icon>add</v-icon>
              </v-btn>
            </template>
      ...
      <v-col xs="16" sm="16" md="14" lg="12" xl="9">
        ...
        <v-data-table :headers="headers" :items="workstationconfigurations" @dblclick:row="(event,{item})=>openEditWorkstationConfiguration(item)"
          ...
          <template v-slot:item.action="{item}">
            <v-tooltip bottom>
              <template v-slot:activator="{on}">
                <v-icon v-can:edit.hide="'workstationsetup'" color="info" v-on="on" small class="mr-2" @click="openEditWorkstationConfiguration(item)">
                  edit
                </v-icon>
              </template>
              <span>Editar</span>
            ...
            <v-tooltip bottom>
              <template v-slot:activator="{on}">
                <v-icon v-can:delete.hide="'workstationsetup'" color="error" v-on="on" small class="mr-2" @click="openRemoveModal(item)">
                  delete
                </v-icon>
              </template>
              <span>Eliminar</span>
